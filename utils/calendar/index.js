...
// time.
const TOKEN_PATH = path.join(process.cwd(), "./utils/calendar/token.json");
const CREDENTIALS_PATH = path.join(
  process.cwd(),
  "./utils/calendar/credentials.json",
);
...
async function createEvent(auth, properties, title) {
  ...
  const startDate = properties.find(
    (property) => property.name === "_start",
  ).value;
  const endDate = properties.find((property) => property.name === "_end").value;
  const attendee = properties.find(
    (property) => property.name === "_attendee",
  ).value;
